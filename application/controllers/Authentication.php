...
class Authentication extends Cl_Controller {
    ...
    public function loginCheck() {
        ...
        $login_type = htmlspecialchars($this->input->post($this->security->xss_clean('login_type')));
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $login_pin =htmlspecialchars($this->input->post($this->security->xss_clean('login_pin')));
                    ...
                    if($company_info){
                        if($company_info->is_active==1){
                            ...
                            if($is_payment_clear=="Yes" && $is_block=="No"){
                                ...
                                $company_info_session['wlb'] =$company_info->wlb;
                                ...
                                if(isFoodCourt() && $user_information->id!=1){
                                    if(isset($user_information->outlet_id) && $user_information->outlet_id){
                                        $outlet_info = $this->db->query("SELECT * FROM tbl_outlets WHERE del_status='Live' AND id=$user_information->outlet_id")->row();
                                    }else{
    ...
    public function changePin() {
        ...
        $old_pin =htmlspecialchars($this->input->post($this->security->xss_clean('old_pin')));
        $new_pin =htmlspecialchars($this->input->post($this->security->xss_clean('new_pin')));
        if($old_pin!=$new_pin){
    ...
    public function SMSSetting($id='') {
        ...
        if ($this->input->post('submit')) {
            ...
            $sms_service_provider = htmlspecialchars($this->input->post($this->security->xss_clean('sms_service_provider')));
            ...
            if ($this->form_validation->run() == TRUE) {
                $sms_info_json = array();
                $sms_info_json['field_1_0'] =htmlspecialchars($this->input->post($this->security->xss_clean('field_1_0')));
                $sms_info_json['field_1_1'] =htmlspecialchars($this->input->post($this->security->xss_clean('field_1_1')));
                $sms_info_json['field_1_2'] =htmlspecialchars($this->input->post($this->security->xss_clean('field_1_2')));

                $sms_info_json['field_2_0'] =htmlspecialchars($this->input->post($this->security->xss_clean('field_2_0')));
                $sms_info_json['field_2_1'] =htmlspecialchars($this->input->post($this->security->xss_clean('field_2_1')));
                $sms_info_json['field_2_2'] =htmlspecialchars($this->input->post($this->security->xss_clean('field_2_2')));
                $sms_info_json['field_2_3'] =htmlspecialchars($this->input->post($this->security->xss_clean('field_2_3')));

                $sms_info = array();
                $sms_info['sms_service_provider'] =htmlspecialchars($this->input->post($this->security->xss_clean('sms_service_provider')));
  ...
  public function setOutlet($outlet_id,$company_id){
    ...
    public function put_customer_panel_data() {
        ...
        $data_item_other['total_item'] = trim($order_details->total_items_in_cart);
        $data_item_other['total_item_1'] = trim($order_details->total_items_with_qty_in_cart);
        $data_item_other['total_sub_total'] = trim($order_details->sub_total);;
        $data_item_other['total_discount'] = trim($order_details->actual_discount);
        $data_item_other['total_tax'] = trim($order_details->total_vat);
        $data_item_other['total_charge'] = trim($order_details->delivery_charge);
        $data_item_other['total_tips'] = trim($order_details->total_tips);
        $data_item_other['total_payable'] = trim($order_details->total_payable);
    ...
    public function securityQuestion(){
        ...
        if ($this->input->post('submit') == 'submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                $security_question = htmlspecialchars($this->input->post($this->security->xss_clean('question')));
                $security_answer = htmlspecialchars($this->input->post($this->security->xss_clean('answer')));
    ...
    public function forgotPasswordStepTwo() {
        $data['matchQuestion'] = htmlspecialchars($this->input->post($this->security->xss_clean('matchQuestion')));
        $data['matchAnswer'] = htmlspecialchars($this->input->post($this->security->xss_clean('matchAnswer')));
        $data['id'] = htmlspecialchars($this->input->post($this->security->xss_clean('id')));
        $json = file_get_contents("./assets/sampleQustions.json");
        $obj  = json_decode($json);
        $data['question'] = $obj;
        ...
        if ($this->form_validation->run() == TRUE) {
            $answer = htmlspecialchars($this->input->post($this->security->xss_clean('answer')));
            $question = htmlspecialchars($this->input->post($this->security->xss_clean('question')));

            $matchQuestion = htmlspecialchars($this->input->post($this->security->xss_clean('matchQuestion')));
            $matchAnswer = htmlspecialchars($this->input->post($this->security->xss_clean('matchAnswer')));
    ...
    public function forgotPasswordStepDone() {
        ...
        if ($this->form_validation->run() == TRUE) {
            $password = htmlspecialchars($this->input->post($this->security->xss_clean('password')));
            $user_id = htmlspecialchars($this->input->post($this->security->xss_clean('id')));
    ...
    public function set_collapse() {
        $value = htmlspecialchars($this->input->post($this->security->xss_clean('status')));
    ...
    public function selectedKitchenPrinting(){
        $selected_order_no = htmlspecialchars($this->input->post($this->security->xss_clean('selected_order_no')));
        //this is an array field that's why we skip the escape
        ...
        if($printers){
            ...
            foreach ($printers as $ky=>$value){
                if(isset($value->id) && $value->id){
                    $is_printing_return++;
                    $sale_items = $this->Common_model->getAllKitchenItems($sale_details->id,$value->id);
                ...
                foreach((Object)$sale->items as $r=>$value){
                    ...
                    if($value->modifiers_name){
                        ...
                        foreach($modifier_names as $ky=>$modifier){
                            $items.= "   ".printLine((getPlanData(trim($modifier_names[$ky]))).": " .(getAmt($modifiers_price[$ky])), ($printer_tmp->characters_per_line - 3))."\n";
                        }
                    }
                }
        ...
        if($sale_id){
            ...
            if(isset($company->receipt_printer_invoice) && $company->receipt_printer_invoice){
                ...
                $payments = "";
                    ...
                    $currency_type = trim($sale->is_multi_currency);
                    $multi_currency = trim($sale->multi_currency);
                    $multi_currency_rate = trim($sale->multi_currency_rate);
                    $multi_currency_amount = trim($sale->multi_currency_amount);
                    if($currency_type==1){
    ...
    public function add_reservation() {
        $data = array();
        $data['company_id'] =htmlspecialchars($this->input->post($this->security->xss_clean('company_id')));
        $data['outlet_id'] =htmlspecialchars($this->input->post($this->security->xss_clean('outlet_id')));
        ...
        $data['number_of_guest'] =htmlspecialchars($this->input->post($this->security->xss_clean('number_of_guest')));
        $data['reservation_date'] =htmlspecialchars($this->input->post($this->security->xss_clean('reservation_date')));
        $data['reservation_type'] =htmlspecialchars($this->input->post($this->security->xss_clean('reservation_type')));
        $data['special_request'] =htmlspecialchars($this->input->post($this->security->xss_clean('special_request')));
    ...
    public function changeReservation() {
        $id = htmlspecialchars($this->input->post($this->security->xss_clean('id')));
        $status = htmlspecialchars($this->input->post($this->security->xss_clean('status')));
